...
def validUTF8(data):
    ...
    for c in data:
        ...
        if c >= 0x00 and c <= 0x7f:
            ...
            valid_flags = [c_bytes[0] <= 0b0111_1111]
        if c >= 0x80 and c <= 0x7ff:
            c_bytes = struct.pack(">H", c)
            valid_flags = [
                c_bytes[0] <= 0b1011_1111,
                c_bytes[1] <= 0b1101_1111
            ]
        if c >= 0x800 and c <= 0xffff:
            c_bytes = struct.pack(">I", c)
            valid_flags = [
                (c_bytes[1] <= 0b1011_1111),
                (c_bytes[2] <= 0b1011_1111),
                (c_bytes[3] <= 0b1110_1111)
            ]
